...
namespace eval ::Gpu {
    ...
    proc draw {pipeline args} {
        # The first push constant is actually the bounding quad; it is
        # a push constant of the autogenerated vertex shader that we
        # provide.
        set args [linsert $args 0 {{-1 -1} {1 -1} {-1 1} {1 1}}]
    ...
    proc pipeline {args} {
        ...
        if {![info exists vertShaderModule]} {
            set vertShaderModule [createShaderModule [glslc -fshader-stage=vert {
                ...
                layout(push_constant) uniform Quad { mat4x2 quad; };
        ...
        set args [linsert $args 0 mat4x2 quad]
