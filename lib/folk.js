...
${message} in environment [list [list this $chan __ctx $ctx __seq ${seqNumber}]]`;
  ...
  async watchCollected(statement, onChange) {
    ...
    } in environment [list [list this $this __ctx $__ctx]]`);
  ...
  async subscribe(statement, onPublish) {
    ...
    const retractKey = await this.send(tcl`
      ...
      } in environment [list [list __ctx $__ctx __varNames $__varNames __wsLib $::wsLib]]
    `);
    channel.retractKey = retractKey;
