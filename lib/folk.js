...
Assert! when websocket $chan is connected \
  [list {} [list apply {{this __ctx __seq} ${message}} $chan $ctx [incr ::__seq]]] \
  with environment {}`);
  }
  ...
  async watchCollected(statement, onChange) {
    ...
    await this.send(tcl`Say when the collected matches for ${statement} are /matches/ [list {match} [list apply {{this __ctx} {
      upvar match match
  ...
  async watch(statement, callbacks) {
    ...
    await this.send(tcl`
      ...
      Say when {*}${statement} [list {args} [list apply {{this ctx names} {
        upvar args args
