...
class FolkWS {
  ...
  async watchCollected(statement, onChange) {
    ...
    await this.send(tcl`Say when the collected matches for ${statement} are /matches/ {{this ctx match} {
      if {![info exists ::wsLib]} { set ::wsLib [dict get [lindex [Query! /someone/ claims the websocket library is /wsLib/] 0] wsLib] }
      $::wsLib wsWrite $ctx [list ${channel.prefix} $match]
    }} with environment [list $this $ctx]`);
  ...
  async watch(statement, callbacks) {
    ...
    await this.send(tcl`
      ...
      Say when {*}${statement} {{this ctx names args} {
        ...
        if {![info exists ::wsLib]} { set ::wsLib [dict get [lindex [Query! /someone/ claims the websocket library is /wsLib/] 0] wsLib] }
        $::wsLib wsWrite [list ${channel.prefix} [list add $matches $::matchId]]
        On unmatch {
          if {![info exists ::wsLib]} { set ::wsLib [dict get [lindex [Query! /someone/ claims the websocket library is /wsLib/] 0] wsLib] }

          $::wsLib wsWrite [list ${channel.prefix} [list remove $matches $::matchId]]
        }
      }} with environment [list $this $ctx $varNamesWillBeBound]
    `);
    return channel;
  }
}
