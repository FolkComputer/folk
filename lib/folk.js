...
class FolkWS {
  ...
  async send(message) {
    await this.connected;
    this.ws.send(tcl`Assert when websocket $chan is connected [list {this __seq} ${message}] with environment [list $chan [incr ::__seq]]`);
  }
  // Evaluates inside a persistent match context that replaces any
  // previous commit with same key:
  async commit(key, program) {
    await this.connected;
    this.ws.send(tcl`Commit (on $chan) ${key} ${program}`);
  }
