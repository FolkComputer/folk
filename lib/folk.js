...
const eatBrace = (str, op='{', cl='}') => {
  ...
  throw new Error('missing closing "}" or "]"');
};
...
class FolkWS {
  ...
  async watch(statement, callbacks) {
    const channel = this.createChannel((message) => {
      ...
      callback && callback(loadList(match), matchId);
    });

    await this.send(tcl`Say when {*}${statement} {{this args} {
      emit ${channel.prefix} [list add $args $::matchId]
      On unmatch {
        emit ${channel.prefix} [list remove $args $::matchId]
      }
    }} with environment [list $this]`);
