...
class FolkWS {
  ...
  async watchCollected(statement, onChange) {
    ...
    await this.send(tcl`Say when the collected matches for ${statement} are /matches/ {{this __ctx match} {
      ...
      $::wsLib wsQueueMsg $__ctx [list ${channel.prefix} $match]
  ...
  async watch(statement, callbacks) {
    ...
    await this.send(tcl`
      ...
      Say when {*}${statement} {{this __ctx names args} {
        set matchId [__currentMatchId]
        ...
        if {![info exists ::wsLib]} { set ::wsLib [dict get [lindex [Query! /someone/ claims the websocket library is /wsLib/] 0] wsLib] }
        $::wsLib wsQueueMsg $__ctx [list ${channel.prefix} [list add $matches $matchId]]
        On unmatch {
          if {![info exists ::wsLib]} { set ::wsLib [dict get [lindex [Query! /someone/ claims the websocket library is /wsLib/] 0] wsLib] }

          $::wsLib wsQueueMsg $__ctx [list ${channel.prefix} [list remove $matches $matchId]]
        }
