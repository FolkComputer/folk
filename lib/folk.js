...
class FolkWS {
  ...
  async send(message) {
    ...
    await this.evaluate(tcl`\
Assert! when websocket $chan is connected \
  [list apply {{this __ctx __seq} ${message}} $chan $ctx [incr ::__seq]] \
  with environment {}`);
  }
  ...
  async watchCollected(statement, onChange) {
    ...
    await this.send(tcl`Say when the collected matches for ${statement} are /match/ [list apply {{this __ctx} {
      upvar match match
  ...
  async watch(statement, callbacks) {
    ...
    await this.send(tcl`
      ...
      Say when {*}${statement} [list apply {{this ctx names} {
        set matchRef [__currentMatchRef]
        set matches [dict create]
        foreach varName $names {
          dict append matches $varName [uplevel [list set $varName]]
        }
      ...
      }} $this $__ctx $varNamesWillBeBound] with environment {}
    `);
    return channel;
  }
}
